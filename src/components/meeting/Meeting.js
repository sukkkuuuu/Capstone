...
const Meeting = ({isLogin}) => {
    ...
    useEffect(() => {
        ...
        const fetchBoards = async () => {
            try{
                const response = await axios.get(Constant.BASE_URL + `/meeting/boards?page=${currentPage}`);
            ...
            {isLoading ? <Loading /> : 
                <div className={styles.board_container}>
                    <div>
                        ...
                        <span className={`${styles.board_field} ${styles.board_writer} ${styles.board_field_title}`}>작성자</span>
                        <span className={`${styles.board_field} ${styles.board_date} ${styles.board_field_title}`}>게시 날짜</span>
                    </div>
                    ...
                    {isLogin && <div className={styles.btn} onClick={() => navigate("create")}>게시글 작성</div>}
                </div>
            }
